...
def predict_downsampled(points, semantic_model, instance_model,dist=5):
    ...
    ply_semantic = create_ply_pcd_from_points_with_labels(points,pred_semantic_label_labels)
...
def pred_full_size(full_points, downsampled_points, downsampled_semantic, downsampled_instance, k=10):
...
def load_model(model_name,version):
    if version == -1:
        versions = os.listdir(f"/space/ariyanzarei/sorghum_segmentation/models/model_checkpoints/{model_name}/lightning_logs")
...
def main_ds(args):
    ...
    points,_,_,_,_ = load_test_data(f"/space/ariyanzarei/sorghum_segmentation/dataset/2022-03-10/sorghum__labeled_{args.set}.hdf5",args.index)
    
    painted_pcd_is_focal,painted_pcd_initial_cluster,_,_ = predict_downsampled(points,semantic_model,instance_model)

    save_predicted(painted_pcd_is_focal,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_semantic.ply")
    save_predicted(painted_pcd_initial_cluster,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_instance.ply")
...
def main_ply(args):
    ...
    path = f"/space/ariyanzarei/sorghum_segmentation/dataset/2022-03-10/PointCloud/{args.index}.ply"
    pcd = load_pcd_plyfile(path)
    points = torch.Tensor(pcd['points'])
    points_full = torch.Tensor(pcd['points_full'])

    downsampled_semantic_pcd,downsampled_instance_pcd,downsampled_semantics,downsampled_instance = \
        predict_downsampled(points,semantic_model,instance_model)

    semantic_pcd,instance_pcd = pred_full_size(points_full,points,downsampled_semantics,downsampled_instance)

    save_predicted(downsampled_semantic_pcd,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_semantic.ply")
    save_predicted(downsampled_instance_pcd,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_instance.ply")
    save_predicted(semantic_pcd,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_semantic_full.ply")
    save_predicted(instance_pcd,f"/space/ariyanzarei/sorghum_segmentation/results/{args.index}_instance_full.ply")
